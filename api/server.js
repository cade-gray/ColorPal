...
var Palette = require("./paletteModel.js")
...
app.get('/login/:username/:password', async function (req, res) {
  try{  
    ...
    colorPalDB.on('error', console.error.bind(console, 'MongoDB connection error:'));
...
app.post('/palettes/:name/:creator/:count/:color1/:color2/:color3/:color4/:color5/', async function (req, res) {
  try{
...
app.get('/likes/:userid', async function (req, res) {
  try{
    ...
    var colorPalDB = mongoose.connection
    colorPalDB.on('error', console.error.bind(console, 'MongoDB connection error:'));
    const query = await Like.find({user_id: userID}).populate("palette_id").select("palette_id")
    res.json(query)
  }
  catch(error){
    console.log("Something went wrong")
...
app.delete('/likes/:userid/:paletteid', async function (req, res) {
  try{
    ...
    var colorPalDB = mongoose.connection
    colorPalDB.on('error', console.error.bind(console, 'MongoDB connection error:'));
    var result = await Like.deleteOne({ user_id: userid, palette_id: paletteid });
    if(result){
